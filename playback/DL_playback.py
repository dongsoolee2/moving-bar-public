# DL_playback(recorded file *.h5, threshold, interval)

# DL_playback function calculates current to be injected
# to amplify or diminish membrane potential of targeted neuron.
#
# by Dongsoo Lee (created 17-02-23, edited 17-03-15)
...
duration = np.int(np.round(np.min(max_diff)))
print(duration)
...
# added 17-07-18 (normalize the injecting current; set the maximum to 1 (nA)
...
current_set = np.concatenate((np.zeros(len(ccs)), ccs, -ccs, ccs, -ccs), axis=0)
#                                                  a     d    a     d
